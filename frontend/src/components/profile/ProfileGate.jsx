...
function ProfileAvatar({ profile, size = 'large' }) {
  const dimension = size === 'small' ? 'h-10 w-10 text-lg' : 'h-28 w-28 text-5xl md:h-36 md:w-36 md:text-6xl';
  return (
    <div
      className={`${dimension} grid place-items-center rounded-md font-black text-white shadow-2xl ring-2 ring-white/10 transition-colors`}
...
export default function ProfileGate({ children, disabled = false }) {
  ...
  const [form, setForm] = useState({ name: '', avatar_color: AVATAR_COLORS[0], is_kids: false });
  ...
  const loadProfiles = useCallback(async () => {
    ...
    try {
      ...
      if (stored?.id && !list.some((profile) => Number(profile.id) === Number(stored.id))) {
        clearActiveProfile();
      }
    } catch (err) {
  ...
  const openCreate = () => {
    ...
    setForm({ name: '', avatar_color: AVATAR_COLORS[profiles.length % AVATAR_COLORS.length], is_kids: false });
    setError('');
  };
    ...
    <div className="fixed inset-0 z-[10000] min-h-screen overflow-y-auto bg-[#141414] text-white">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col items-center justify-center px-6 py-10">
        <div className="mb-10 text-center">
          ...
          <p className="mt-3 text-sm text-white/50 md:text-base">Chọn profile để giữ riêng danh sách, lịch sử xem và gợi ý phim.</p>
        </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-[10001] grid place-items-center bg-black/75 px-4 backdrop-blur-sm">
          <form onSubmit={saveProfile} className="w-full max-w-md rounded-xl border border-white/10 bg-[#181818] p-6 shadow-2xl">
            <h2 className="mb-5 text-2xl font-bold">{editing.mode === 'edit' ? 'Quản lý profile' : 'Thêm profile'}</h2>
            <div className="mb-5 flex items-center gap-4">
              <ProfileAvatar profile={{ name: form.name, avatar_color: form.avatar_color }} size="small" />
              <input
                value={form.name}
            ...
            <div className="mb-5">
              <div className="mb-2 text-sm font-semibold text-white/60">Màu avatar</div>
            ...
            <label className="mb-6 flex cursor-pointer items-center justify-between rounded-lg bg-white/5 px-4 py-3">
              <span>
                <span className="block font-semibold">Profile trẻ em</span>
                <span className="block text-xs text-white/45">Dành cho nội dung nhẹ nhàng hơn.</span>
              </span>
              <input
                type="checkbox"
                checked={form.is_kids}
                onChange={(event) => setForm((current) => ({ ...current, is_kids: event.target.checked }))}
